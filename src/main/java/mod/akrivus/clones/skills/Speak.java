...
public class Speak extends SkillBase {
		...
		for (int sentence = 0; sentence < sentences.length; ++sentence) {
			for (int word = 0; word < sentences[sentence].length; ++word) {
				if (word % 2 == 0) {
					...
					if ((pos.startsWith("NN") || pos.startsWith("PR")) && (TRIGGER_NOUNS.contains(sentences[sentence][word]) || TRIGGER_NOUNS.isEmpty())) {	
					...
					if (pos.startsWith("VB") && (TRIGGER_VERBS.contains(sentences[sentence][word]) || TRIGGER_VERBS.isEmpty())) {
