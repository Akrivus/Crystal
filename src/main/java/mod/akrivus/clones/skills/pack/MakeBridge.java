...
public class MakeBridge extends Speak {
	private float[] direction = new float[] { 0, 0 };
	private Block bridgeBlock = Blocks.COBBLESTONE;
		...
		if (previous) {
			...
			for (String subject : this.collectedSubjects) {
				...
				if (item instanceof ItemBlock) {
					this.bridgeBlock = Block.getBlockFromItem(item);
				}
			}
		}
		return previous;
	...
	public void run(EntityClone clone) {
		if (this.lastBlockPlace > clone.getPlaceSpeed()) {
			BlockPos nextPos = clone.getPosition().add(this.direction[0], -1, this.direction[1]);
			clone.lookAt(nextPos.add(this.direction[0], -1, this.direction[1]));
			boolean placed = false;
			if (!clone.world.getBlockState(nextPos.down()).getBlock().isBlockSolid(clone.world, nextPos, EnumFacing.UP)) {
				placed = clone.placeBlock(this.bridgeBlock, nextPos);
				if (placed) {
					clone.setItemClass(Item.getItemFromBlock(this.bridgeBlock));
				}
			}
