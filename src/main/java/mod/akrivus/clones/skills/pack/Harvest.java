...
public class Harvest extends Speak {
			...
			for (int y = -8; y < 8; ++y) {
				for (int z = -16; z < 16; ++z) {
					...
					if (this.isCorrectFarmBlock(state)) {
						BlockPos above = clone.getPosition().add(x, y + 1, z);
						if (this.isCorrectPlant(clone.world.getBlockState(above)) || clone.world.isAirBlock(above)) {
							plants.add(above);
						}
					}
				}
		...
		if (this.harvestLocation != null) {
			...
			if (clone.getDistanceSqToCenter(this.harvestLocation) < clone.getBuildRange()) {
				...
				if (this.lastBlockBreak > clone.getHarvestSpeed(harvest)) {
					...
					if (clone.world.isAirBlock(this.harvestLocation)) {
						clone.setItemClass(this.getSeed());
						success = clone.world.mayPlace(this.getPlant(), this.harvestLocation, true, EnumFacing.UP, null);
						if (success) {
							success = clone.removeItemFromInventory(this.getSeed());
							if (success) {
								success = clone.world.setBlockState(this.harvestLocation, this.getPlant().getDefaultState(), 3);
							}
						}
					}
